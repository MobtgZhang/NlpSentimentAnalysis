...
def add_train_args(parser):
    ...
    files.add_argument('--train-file', type=str,
                       default='training-processed-pyltp.json',
...
def set_defaults(args):
    ...
    if args.words_embedding_file:
        '''
        import gensim
        model=gensim.models.KeyedVectors.load_word2vec_format(args.words_embedding_file)
    ...
    if args.char_embedding_file:
        '''
        import gensim
        model = gensim.models.KeyedVectors.load_word2vec_format(args.char_embedding_file)
        dim = model.vector_size
        del model
        '''
        args.embedding_dim = 300
...
def validate_official(args, data_loader, model, global_stats,saver):
    ...
    for ex in data_loader:
        ...
        examples += batch_size
    saver.add_loss(loss.data)
    saver.add_f1(f1_score_avg.avg)
    saver.add_em(exact_match.avg)
    logger.info('%s valid unofficial: Epoch = %d | f1_score = %.2f | ' %
                (saver.mode, global_stats['epoch'], f1_score_avg.avg) +
                'exact = %.2f | examples = %d | ' %
                (exact_match.avg, examples) +
                'valid time = %.2f (s)' % eval_time.time())
    return {'exact_match': exact_match.avg,"f1_score":f1_score_avg.avg}
