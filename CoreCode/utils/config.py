...
RAW_DIR = os.path.join(home,"data","raw_data")
ANNTOTORS = {"pos","ner"}
LTP_MODEL_PATH = os.path.join(home,"ltp_data_v3.4.0")
LABEL_LILST = "id,content," \
              "location traffic convenience,location distance from business district,location easy to find," \
              "service wait time,service waiters attitude,service parking convenience,service serving speed," \
              "price level,price cost effective,price discount," \
              "environment decoration,environment noise,environment space,environment cleaness," \
              "dish portion,dish_taste,dish look,dish recommendation," \
              "others overall experience,others willing to consume again"
CLASS_LIST = "not mentioned,bad,moderate,best"
...
MODEL_ARCHITECTURE = {
    ...
    "first_grained_size","second_grained_size","class_size",'attention_hops','attention_unit','num_heads'
}
...
def add_model_args(parser):
    ...
    model.add_argument('--model-type', type=str, default='rnn',
                       help='Model architecture type: rnn, r_net, mnemonic,sa_encoder')
    model.add_argument('--embedding-dim', type=int, default=300,
                       help='Embedding size if embedding_file is not given')
    model.add_argument('--char-embedding-dim', type=int, default=300,
    ...
    model.add_argument('--attention-hops', type=int, default=30,
                       help='attention-hops')
    model.add_argument('--attention-unit', type=int, default=50,
                       help='attention-unit')
    model.add_argument('--num-heads', type=int, default=2,
