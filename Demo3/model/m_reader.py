...
class MnemonicReader(nn.Module):
    ...
    def __init__(self,args, normalize= True):
        ...
        # Highway rnn genrate word encoder
        self.highway_rnn = layers.RecurrentHighway(
            input_size=args.embedding_dim,
            hidden_size=args.hidden_size,
            layers = args.doc_layers
        )
        ...
        sfu_hidden_size = 2 * args.hidden_size + args.doc_layers * args.hidden_size
    ...
    def forward(self, x, x_c, x_f, x_mask):
        ...
        # Combine input
        crnn_input = [x_emb, x_c_features]
        # Add manual features
        if self.args.num_features > 0:
            crnn_input.append(x_f)
        # Encode document with RNN
        document = self.encoding_rnn(torch.cat(crnn_input, 2), x_mask)
        w_document = self.highway_rnn(x_emb)
        # Align and aggregate
        c_check = document
        w_check = w_document
        for i in range(self.args.hop):
            c_bar = self.interactive_SFUs[i].forward(c_check,torch.cat([c_check,w_check],2))
